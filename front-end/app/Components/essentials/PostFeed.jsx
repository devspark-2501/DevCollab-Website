...
// ─── POST CARD (with real likes + comments) ──────────────────────────────────
function PostCard({ post: initialPost, animate, userEmail, userName }) {
  ...
  // ── Real like — saves to DB, uses server count back
...
export default function Feed() {
  ...
  }, [userEmail]); // re-run once session is ready so liked state is correct
    ...
    <div className="flex min-h-screen bg-[#0b0f1a] text-[#e2e4ec]">
      ...
      <div className="flex-1 min-w-0 overflow-y-auto relative">
        ...
        <div className={`relative z-10 max-w-[680px] mx-auto px-4 sm:px-6 py-8 ${fadeIn}`}>
          ...
          {loading && <div className="space-y-4">{[1,2,3,4,5].map((n) => <SkeletonPost key={n} />)}</div>}
