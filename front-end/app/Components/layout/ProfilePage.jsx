...
export default function ProfileLayout({
  ...
  // ref to scroll to + ref to tell PostFeed to open modal
  ...
  function handleAddPost() {
    // scroll down to post section smoothly
    postSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
    // slight delay so scroll finishes before modal pops
      ...
      <div className="flex-1 overflow-y-auto flex flex-col min-w-0">
        ...
        <div className="relative">
          ...
          <div className="max-w-[820px] w-full mx-auto px-4 sm:px-8 pb-10">
            ...
            <div className={`flex items-end justify-between -mt-10 sm:-mt-12 mb-8 flex-wrap gap-4 ${fadeUp}`} style={{ transitionDelay: "0.05s" }}>
              ...
              {/* ── Buttons: Edit + Add Post + Share ── */}
              <div className="flex gap-2 pb-1.5 w-full sm:w-auto flex-wrap">
                <button className="flex-1 sm:flex-none px-4 py-2 rounded-lg text-[13px] font-medium bg-[#1d2b5c] border border-[#2a3a7a] text-[#8ba4f5] hover:bg-[#22336e] transition-colors">
                  Edit Profile
                </button>
                {/* ── Add Post button ── */}
                <button
                  onClick={handleAddPost}
                  className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-medium
                             bg-[#1d2b5c] border border-[#2a3a7a] text-[#8ba4f5] hover:bg-[#22336e] transition-colors"
                >
                  <Icon size={13}>
                ...
                <button className="flex-1 sm:flex-none px-4 py-2 rounded-lg text-[13px] font-medium bg-transparent border border-[#1e2029] text-[#4a4f62] hover:bg-[#161820] hover:text-[#8ba4f5] transition-colors">
                  Share
                </button>
              </div>
            </div>
        ...
        >
          ...
          <div className="relative z-10 max-w-[820px] w-full mx-auto px-4 sm:px-8 py-10">
            ...
            {/* section hero label — same pattern as community page */}
            ...
            {/* PostFeed gets triggerModal so it can auto-open */}
            <PostFeed
