        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 min-h-0">
          {comments.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-[12px] text-[#2e3244]">No replies yet. Be the first!</p>
            </div>
          )}
          {comments.map((c) => (
            ...
            <div className="flex gap-2.5 items-start">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between mt-1.5">
                  ...
                  <button onClick={handleSubmit} disabled={posting || !commentText.trim()}
                    ...
                    {posting
                      ...
                      : <Icon size={11}><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></Icon>
                    }
                    Reply
                  </button>
                </div>
...
function PostCard({ post: initialPost, userEmail, userName }) {
  const [post, setPost]           = useState(initialPost);
  const [liked, setLiked]         = useState(() => initialPost.likedBy?.includes(userEmail) ?? false);
  ...
  return (
    <>
      <div className="bg-[#13161f] border border-[#1e2029] rounded-2xl flex flex-col
                      hover:border-[#2a3060] hover:bg-[#14172a] transition-all duration-200
                      w-[300px] sm:w-[360px] shrink-0 group">

        {/* card top accent line */}
        <div className={`h-[3px] rounded-t-2xl ${colorCls.split(" ")[0].replace("bg-", "bg-").replace("/", "/")} opacity-60`} />
        ...
        <div className="p-5 flex flex-col flex-1">
          ...
          <p className="text-[13.5px] text-[#8a8fa8] leading-relaxed whitespace-pre-wrap
                        break-words flex-1"
             style={{ display: "-webkit-box", WebkitLineClamp: 5,
                      WebkitBoxOrient: "vertical", overflow: "hidden" }}>
            {post.userContent}
          </p>

          {/* footer */}
          <div className="flex items-center gap-1 mt-5 pt-3.5 border-t border-[#191b24]">
            {/* like */}
            <button onClick={handleLike}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11.5px]
                          font-medium transition-all border
                          ${liked
                            ? "bg-[#1d2b5c]/60 text-[#8ba4f5] border-[#2a3a7a]/50"
                            : "text-[#3a4470] border-transparent hover:bg-[#161820] hover:text-[#8ba4f5]"}`}>
            ...
            <button onClick={() => setModalOpen(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11.5px]
                         font-medium text-[#3a4470] border border-transparent
...
function SkeletonCard() {
  return (
    <div className="bg-[#13161f] border border-[#1e2029] rounded-2xl
                    w-[300px] sm:w-[360px] shrink-0 animate-pulse overflow-hidden">
      <div className="h-[3px] bg-[#1e2029]" />
      <div className="p-5">
        ...
        <div className="space-y-2 mb-6">
          ...
          <div className="h-2.5 bg-[#1e2029] rounded w-4/6" />
          <div className="h-2.5 bg-[#1e2029] rounded w-3/6" />
        </div>
        <div className="border-t border-[#191b24] pt-3.5 flex gap-2">
          <div className="h-7 w-14 bg-[#1e2029] rounded-lg" />
          <div className="h-7 w-16 bg-[#1e2029] rounded-lg" />
        </div>
      </div>
    </div>
  );
...
export default function Hero_Section_Two() {
  ...
  function scrollCards(dir) {
    scrollRef.current?.scrollBy({ left: dir * (360 + 16) * 2, behavior: "smooth" });
  }
  ...
  const totalContributors = [...new Set(posts.map((p) => p.userEmail))].length;

  return (
    <section className="relative bg-[#0b0f1a] overflow-hidden py-24">

      {/* glows */}
      <div className="absolute top-[-80px] right-[-80px] w-[500px] h-[500px] bg-blue-500 opacity-[0.06] blur-[130px] rounded-full pointer-events-none" />
      <div className="absolute bottom-0 left-[-80px] w-[500px] h-[500px] bg-purple-600 opacity-[0.06] blur-[130px] rounded-full pointer-events-none" />
      ...
      <div className="relative z-10">
        ...
        <div ref={headerRef}
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-[52px] font-bold text-[#ebedf5]
                         tracking-tight leading-[1.1] mb-5">
            Community{" "}
            <span className="text-[#8ba4f5]">Discussions</span>
          </h2>

          <p className="text-[14px] sm:text-[15px] text-[#5a5f72] leading-relaxed max-w-lg mx-auto mb-8">
          ...
          {!loading && posts.length > 0 && (
            <div className="flex items-center justify-center gap-2.5 flex-wrap">
              {[
                { val: posts.length,       label: "Posts",        icon: <><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></> },
                { val: totalContributors,  label: "Contributors", icon: <><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></> },
                { val: totalLikes,         label: "Likes",        icon: <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/> },
              ].map((s) => (
                <div key={s.label}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl
                             bg-[#13161f] border border-[#1e2029]
                             hover:border-[#2a3060] transition-colors">
                  <div className="text-[#3a4470]"><Icon size={12}>{s.icon}</Icon></div>
                  <span className="text-[15px] font-semibold text-[#ebedf5] font-mono">{s.val}</span>
                  <span className="text-[10px] text-[#2e3244] uppercase tracking-wider">{s.label}</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* ── DIVIDER ── */}
        <div className="flex items-center gap-4 px-4 sm:px-8 mb-8">
          <div className="flex-1 border-t border-[#1e2029]" />
          <span className="text-[10px] text-[#2e3244] uppercase tracking-widest">Latest posts</span>
          <div className="flex-1 border-t border-[#1e2029]" />
        </div>
        ...
        {!loading && posts.length > 0 && (
          <div className="flex items-center justify-between px-4 sm:px-8 mb-5">
            <p className="text-[11px] text-[#2e3244] select-none">
              {posts.length} post{posts.length !== 1 ? "s" : ""}
              <span className="mx-1.5 opacity-40">·</span>
              drag or use arrows
            </p>
            <div className="flex items-center gap-1.5">
              <button onClick={() => scrollCards(-1)} disabled={!canScrollLeft}
                className="w-8 h-8 rounded-full bg-[#13161f] border border-[#1e2029]
                           flex items-center justify-center text-[#3a4470]
                           hover:border-[#2a3a7a] hover:text-[#8ba4f5]
                           disabled:opacity-20 disabled:cursor-not-allowed transition-all">
                <Icon size={13}><polyline points="15 18 9 12 15 6"/></Icon>
              </button>
              <button onClick={() => scrollCards(1)} disabled={!canScrollRight}
                className="w-8 h-8 rounded-full bg-[#13161f] border border-[#1e2029]
                           flex items-center justify-center text-[#3a4470]
                           hover:border-[#2a3a7a] hover:text-[#8ba4f5]
                           disabled:opacity-20 disabled:cursor-not-allowed transition-all">
        ...
        <div className="relative">
          ...
          {!loading && !error && posts.length === 0 && (
            <div className="text-center py-20 px-4">
              ...
              <p className="text-[15px] font-medium text-[#4a4f62] mb-1">No discussions yet</p>
